...
void RegFatNtuple::fill( const edm::Event& event,int iNrVert,float iRho,float iNrPUInt,float iNrPUIntTrue,
  ...
  if(iSC){
    if(debug)std::cout<<"RegFatNtuple::fill going to fill SC -1 "<<iSC->energy()<<std::endl;
    sc.fill(*iSC,ecalChanStatus);
    if(debug)std::cout<<"RegFatNtuple::fill going to fill SC -2 "<<iSC->energy()<<std::endl;
...
void SuperClustStruct::fill(const reco::SuperCluster& sc,const EcalChannelStatus& ecalChanStatus)
{
