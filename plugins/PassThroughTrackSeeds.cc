...
//
// Package:    PFTracking
// Class:      PassThroughTrackSeeds
// 
// Original Author:  Michele Pioppi
// March 2010. F. Beaudette. Produce PreId information

#include "LowPtElectrons/LowPtElectrons/plugins/PassThroughTrackSeeds.h"
...
PassThroughTrackSeeds::PassThroughTrackSeeds(const ParameterSet& iConfig, const goodseedhelpers::HeavyObjectCache*):
  pfTransformer_(nullptr),
  conf_(iConfig),
  resMapEtaECAL_(nullptr),
  resMapPhiECAL_(nullptr)
{
  LogInfo("PassThroughTrackSeeds")<<"Electron PreIdentification started  ";
  ...
  LogDebug("PassThroughTrackSeeds")<<"Seeds for GSF will be produced ";

  // no disablePreId_ switch here. The collection will be empty if it is true
  produces<ElectronSeedCollection>(preidgsf_);

  if(produceCkfseed_){
    LogDebug("PassThroughTrackSeeds")<<"Seeds for CKF will be produced ";
    produces<TrajectorySeedCollection>(preidckf_);
  }

  if(producePreId_){
    LogDebug("PassThroughTrackSeeds")<<"PreId debugging information will be produced ";
...
void
PassThroughTrackSeeds::produce(Event& iEvent, const EventSetup& iSetup)
{

  LogDebug("PassThroughTrackSeeds")<<"START event: "<<iEvent.id().event()
  ...
  for (unsigned int istr=0; istr<tracksContainers_.size();++istr){
    ...
    LogDebug("PassThroughTrackSeeds")<<"Number of tracks in collection "
                                <<"tracksContainers_[" << istr << "] to be analyzed "
                                <<Tk.size();

    //loop over the track collection
    for(unsigned int i=0;i<Tk.size();++i){		
      //if (useQuality_ && (!(Tk[i].quality(trackQuality_)))) continue;
      ...
      if(!disablePreId_)
			{
				...
				GoodPreId = (tkpt > minPt_); //just a pt cut

#ifdef EDM_ML_DEBUG      
				if(GoodPreId)
					LogDebug("PassThroughTrackSeeds")<<"Track (pt= "<<Tk[i].pt()<<
						"GeV/c, eta= "<<Tk[i].eta() <<
      ...
      if (true)//producePreId_ && myPreId.pt()>PtThresholdSavePredId_)
			{
...
void 
PassThroughTrackSeeds::beginRun(const edm::Run & run,
													 const EventSetup& es)
{
  //Magnetic Field
  ESHandle<MagneticField> magneticField;
...
int 
PassThroughTrackSeeds::getBin(float eta, float pt){
  int ie=0;
  int ip=0;
  ...
  int iep= ie*3+ip;
  LogDebug("PassThroughTrackSeeds")<<"Track pt ="<<pt<<" eta="<<eta<<" bin="<<iep;
  return iep;
}

void PassThroughTrackSeeds::fillPreIdRefValueMap( Handle<TrackCollection> tracks,
...
DEFINE_FWK_MODULE(PassThroughTrackSeeds);
